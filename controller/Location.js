...
// Configuration
const EXPIRE_DAYS = 7;
const CLEANUP_COOLDOWN_MS = 1000 * 60 * 60;
...
async function expireStaleLocationsIfNeeded() {
  try {
    ...
    console.info(
      `expireStaleLocationsIfNeeded: matched=${res.matchedCount ?? res.n ?? 0} modified=${
...
exports.location = async (req, res) => {
  try {
    ...
    // If client sends 0,0 -> keep previous location and return it
    if (lat === 0 && lng === 0) {
    ...
    if (user && user.lastLocation && user.lastLocation.recordedAt) {
      const oldCoords =
        user.lastLocation.coords &&
        user.lastLocation.coords.coordinates;
    ...
    // Conditional update: only if incoming recordedAt newer
    const filter = {
      _id: userId,
      $or: [
